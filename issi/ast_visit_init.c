...
static void _visitFunctionDef(ast_node n) {
	...
	dbgprintf(
	    "declared function %s with %d parameters\n", fdef->identifier,
...
static void _visitAst(ast_node n) {
	...
	case AST_FUNCTIONRETURN:
		_visitFunctionReturn(n);
		break;
	case AST_DACA:
		_visitBlock(((ast_daca)n->payload)->ifTrue);
		if (((ast_daca)n->payload)->ifFalse != NULL)
			_visitBlock(((ast_daca)n->payload)->ifTrue);
		break;
	case AST_CATTIMP:
		_visitBlock(((ast_catTimp)n->payload)->block);
		break;
	default:
		parsingStop("AST node type %d not handled", n->type);
		break;
	}
}